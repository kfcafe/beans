//! `bn locks` — View and manage file locks.
...
pub fn cmd_locks(beans_dir: &Path) -> Result<()> {
    let active = locks::list_locks(beans_dir)?;
...
pub fn cmd_locks_clear(beans_dir: &Path) -> Result<()> {
    let cleared = locks::clear_all(beans_dir)?;
    if cleared == 0 {
