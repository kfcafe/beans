...
/// Append a completion record to `.beans/agent_history.jsonl`.
///
/// Gracefully swallows errors — logging should never crash the agent.
pub fn append_history(beans_dir: &Path, entry: &AgentHistoryEntry) {
...
mod tests {
    ...
    fn append_creates_file_and_writes_valid_jsonl() {
        ...
        append_history(&beans_dir, &entry);

        let content = fs::read_to_string(beans_dir.join("agent_history.jsonl")).unwrap();
    ...
    fn append_appends_multiple_lines() {
        ...
        append_history(&beans_dir, &make_entry(true));
        append_history(&beans_dir, &make_entry(false));
