...
```bash
git clone https://github.com/kfcafe/beans.git
cd beans
cargo build
cargo test
```
...
### Sorting
Use `util::natural_cmp` when sorting bean IDs so that `2` sorts before `10`.
...
src/
  ...
  commands/
    ...
    close.rs
    ...
  index.rs             # bean index (in-memory + YAML persistence)
  bean.rs              # bean data model
