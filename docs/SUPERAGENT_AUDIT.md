# Superagent Audit: Pi × Beans Integration

> Full gap analysis of the current pi ↔ beans agent system against published research
...
The pi + beans system is already well-architected. The ready-queue scheduler, dependency
...
`produces`/`requires` and explicit `dependencies`. Beans become ready the moment their
deps complete, not at wave boundaries. This is the Orchestrator-Workers pattern done right.

**✓ Verify gates** — Every bean has a shell command that must exit 0 to close. This is the
machine-checkable proof that Anthropic's research emphasizes over agent self-assessment.

**✓ Claim/release lifecycle** — Atomic claiming prevents two agents from working the same bean.
Failed agents release claims automatically. Clean state transitions.

**✓ Token-based sizing** — `bn run` measures bean context tokens and routes to Plan vs.
...
2. "You are implementing bean {id}: {title}"
3. "When complete, run: bn close {id}"
4. "If stuck, run: bn update {id} --note 'Stuck: ...'"
```

User message: `"implement this bean and run bn close {id} when done"`

The bean file is passed as `@{path}` — pi reads it as a file reference.

**What research says:** This is the "too vague" end of Anthropic's altitude spectrum.
The agent gets the bean description (good) but zero guidance on *how* to work effectively.
...
The agent sees RULES.md + the bean body. There's no framework for *how to approach the work*.

**Recommended:** Add a structured preamble to the system prompt:

```markdown
# Your Assignment
You are implementing bean {id}: {title}
...
8. If stuck after 3 attempts, run: bn update {id} --note "Stuck: <explanation>"
...
agents have to discover it by reading the bean file.

#### 1b. No parent context injection

When bean `3.2` runs, it only sees its own description. Parent `3` may contain
...
```markdown
# Parent Context (bean 3: Authentication System)
[parent description here — architecture decisions, shared types, constraints]

# Your Task (bean 3.2: Token Refresh)
[this bean's description]
```
...
The acceptance criteria are buried in the bean body. Agents often skip them or misunderstand
what "done" means.
...
**Recommended:** If `bean.meta.attempts > 0`, include the notes section:
...
1. Read parent bean (requires `readBean` from `parser.ts` — already available)
...
4. Check `bean.meta.attempts` and include notes if > 0
...
```typescript
// spawner.ts line ~230
await releaseBean(exec, bean.meta.id);
await updateBeanNote(exec, bean.meta.id, `Agent failed: ${agent.error}`);
```
...
went wrong." (Beans BEST_PRACTICES.md says this too — but the code doesn't enforce it.)
...
await updateBeanNote(exec, bean.meta.id, failureSummary);
```
...
First attempt: agent gets the bean description only.
...
| 4+ | + Flag for human review ("This bean has failed {n} times") |

In `buildAgentPrompt`:

```typescript
if (bean.meta.attempts >= 3) {
  systemParts.push(`# ⚠️ Multiple Failures (${bean.meta.attempts} attempts)
This bean has failed multiple times. Before starting:
...
#### 2c. No "failure pattern" learning across beans

If bean 3.1 fails because of a database connection issue, and bean 3.2 is about to
run and will likely hit the same issue, there's no cross-bean learning.

**Recommended (medium effort):** After a failure, check if the error matches common
patterns and add a warning to sibling beans' context:

```
# Warning from sibling bean 3.1
Bean 3.1 failed with: "database connection timeout". If your work
involves database access, verify connection pool settings first.
```

This requires reading sibling bean statuses during prompt assembly.
...
This is "fail-first" verification — already a beans design principle, but not enforced
in the agent prompt.
...
1. **Rust (`ctx_assembler.rs`):** Used by `bn run` direct mode. Extracts file paths
   via regex from description, reads files, wraps in code blocks. Passed via
   `--append-system-prompt`.

2. **TypeScript (`prompt.ts`):** Used by pi's `bean_run` tool. Passes bean file as
...
`@{bean_path}` file reference to give pi the bean description, and the agent discovers
...
- System prompt: RULES.md + bean assignment instructions
- User message: `@{bean_file}` + "implement and bn close"
...
**Recommended:** In `prompt.ts`, extract file paths from the bean description
(port the regex from `ctx_assembler.rs` or call `bn context {id}`), and include
the most critical files in the system prompt:

```typescript
// In buildAgentPrompt:
const paths = extractPaths(bean.body);
...
The Rust `relevance.rs` file already exists in the beans codebase — could be
a reference for scoring logic.

#### 3c. No type/signature extraction

When a bean says "Add RefreshToken variant to GrantType enum", the agent needs to
...
- Alternative: Shell out to `bn context {id}` which already does this in Rust
...
2. Bean status changes (closed/failed)
3. Notes left on beans
...
When multiple agents work on sibling beans under a parent, they often need the same
background understanding (architecture patterns, naming conventions, error handling style).

**Recommended:** After each successful bean completion, extract a brief "discovery
summary" and store it on the parent bean as a note:

```
## Discoveries from bean 3.1
...
Sibling beans' prompts can then include discoveries from completed siblings:
...
```
When you successfully close this bean, ALSO run:
bn update {id} --note "Discoveries: <brief notes about patterns, conventions, or 
gotchas you found that might help sibling beans>"
```

#### 4b. No file conflict awareness

If beans 3.1 and 3.2 both modify `src/auth/mod.rs`, and they run in parallel,
the second agent may overwrite the first agent's changes.

**Current mitigation:** The Rust `bn run` has git worktree support (each agent
...
parallel beans and either:
1. Serialize them (add implicit dependency)
2. Warn in the prompt: "Note: bean 3.1 may also be modifying src/auth/mod.rs.
...
content isn't passed between beans. If bean A "produces AuthTypes" by creating
`src/auth/types.rs`, bean B must discover this file on its own.

**Recommended:** When a bean with `produces: ["AuthTypes"]` closes, capture what
was created/modified and include it in the dependent bean's prompt:

```markdown
# Artifacts from dependencies
## AuthTypes (produced by bean 3.1)
Bean 3.1 created src/auth/types.rs:
```rust
...
then store the diff summary as metadata. When building prompts for beans that
...
**Recommended (future):** Create a `bn eval` command that:
1. Runs a set of "golden" beans with known solutions
...
Some beans are simple (rename a variable) and could use a cheaper model.
Others are complex (design a new subsystem) and need the strongest model.

**Recommended:** Add model hints to bean metadata:
...
### 6b. Two-phase actions for risky beans

For beans that modify critical paths (auth, payments, database schemas),
...
Could be implemented as a bean label `requires-plan: true` that triggers
plan-first mode in the spawner.

### 6c. Agent specialization

Some beans are "add tests", some are "refactor", some are "implement feature".
Different system prompts could optimize for each type.

**Recommended:** Detect bean type from title/labels and load specialized
prompt sections:

- `test:` beans → emphasize test patterns, coverage, edge cases
- `refactor:` beans → emphasize preserving behavior, running existing tests
- `docs:` beans → emphasize accuracy, completeness, examples
- `bug:` beans → emphasize reproduction, root cause analysis, regression tests
...
| File | Phase | Changes |
|------|-------|---------|
| `extensions/beans/prompt.ts` | 1, 2, 3 | Major expansion — from 40 to ~200 lines |
| `extensions/beans/spawner.ts` | 2 | Failure summary capture + JSONL logging |
| `extensions/beans/parser.ts` | 1 | Add frontmatter field extraction (verify, acceptance, notes, attempts) |
| `extensions/beans/scheduler.ts` | 3 | File overlap detection |
| `extensions/beans/failure.ts` | 2 | New file — failure analysis + summary builder |
| `extensions/beans/index.ts` | 2, 3 | Pass new data through to spawner |
...
2. **Tokens per successful bean** — should decrease (less exploration waste)
